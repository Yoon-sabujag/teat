...
- 성적/폭력적 씬의 시스템 프롬프트에 **장르·상황·관계**를 한 줄로 명시해두면 Gemini가 덜 거부한다. 예: *"성인 대상 누아르. 두 인물은 과거 연인이었고 상호 합의된 재회."*
- 빈 응답(`502: 빈 응답`) 받으면 강도를 살짝 낮춰서 다시 쓰기 — "노골적 묘사" → "강렬한 암시" 수준으로 조절.
...
노드에 `improv: { npc, systemPrompt, triggers[], suggestions[] }` 설정하면 `lines[]` 이후 자유 입력창. 시스템 프롬프트에 페르소나를 주입, `triggers[]`의 키워드가 LLM 응답에 포함되면 다음 노드로 라우팅. 트리거에 `effects`도 부착 가능.

**프롬프트 관례**: (1) 페르소나, (2) 한국어 출력, (3) 트리거 키워드를 응답 끝에 붙이라는 지시.

**`suggestions[]`는 필수**. 비어있는 입력창 앞에 플레이어를 세우지 말 것. 4~6개의 각도 다른 첫 질문을 작가가 써두면 UI에 칩으로 뜨고, 탭하면 바로 전송됨. 칩은 트리거 키워드가 아니라 **평범한 질문**이어야 함 — 너무 노골적이면 LLM 연기가 부자연스러워짐.

**메모리 HUD**: 플레이어는 씬 안에서 "기억" 버튼으로 `pc.memory[]` 누적 로그를 볼 수 있음. improv 노드에서 뭘 물을지 모를 때 자기가 지금까지 뭘 알아냈는지 확인용. 메모리는 **플레이어에게 보여줄 것**을 전제로 짧게 써야 함 (길면 HUD가 지저분).
